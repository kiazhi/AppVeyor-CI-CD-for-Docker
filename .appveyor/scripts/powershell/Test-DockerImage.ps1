...
if ($SCRIPT:Files -ne (Out-Null))
{
    ...
    { 
        if($_ -like $SCRIPT:DockerFilePathFilter)
        {
            ...
            # Validate test script exist for testing the container
            if(Test-Path -Path "$SCRIPT:DirectoryPath\Test-Container.ps1")
            {
                ...
                if((Get-Content `
                    -Path "$SCRIPT:DirectoryPath\Test-Container.ps1" | `
                    Where-Object `
                    { 
                        ($_ -match '\S') -and 
                ...
                elseif((Get-Content `
                    -Path "$(Split-Path -Path $SCRIPT:DirectoryPath -Parent)\Test-Container.ps1" | `
                    Where-Object `
                    { 
                        ($_ -match '\S') -and
                        ($_.ToString().StartsWith('#') -ne $True) -and
